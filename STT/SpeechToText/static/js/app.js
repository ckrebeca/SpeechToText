...
if (!Array.prototype.find) {
  ...
  var url = "http://206.189.46.10";
  var token = "XA7NZj5hzZGvFgBlCaSEVbRBhF2gxQ01";
  $("#aList li").click(function(){
    var lat = parseFloat($(this).find(".aLat").text());
    var long = parseFloat($(this).find(".aLong").text());
    if (isNaN(lat)) {
        alert("pending");
  ...
  var map = new ol.Map({
      ...
      target: 'map',
      view: new ol.View({
          center: ol.proj.fromLonLat([121.0437, 14.6760]),
          zoom: 2
      })
  });
  ...
  var splitted;
  $.ajax({
      url: "http://206.189.46.10/api/geofences",
      method: "GET",
      ...
      success: function(data){
          ...
          geofences = JSON.parse(geo);
          for (let index = 0; index < geofences.length; index++) {
            sub = geofences[index]['area'].substring(7);
            removed = sub.replace(/[()]/g, '');
            removed = removed.replace(/[,]/g, '');
            splitted = removed.split(" ");
            lat = splitted[0];
            long = splitted[1];
            rad = splitted[2];
            mark = markers[geofences[index]['name']];
            circle = new ol.geom.Circle([lat,long],rad);
  ...
  ajax('GET', url + '/api/server', function(server) {
      ajax('GET', url + '/api/session?token=' + token, function(user) {
          ...
          map.getView().setZoom(user.zoom || server.zoom || 12.5);
